...
/// <summary>
/// Loads and manages JSON configuration files for TrialMatch infrastructure
/// </summary>
public class ConfigurationLoader
{
    ...
    public T SubstituteVariables<T>(T config, DeploymentContext context) where T : class
    {
        ...
        jsonString = jsonString.Replace("${LOG_GROUP_NAME}", context.Namer.LogGroup("trial-match", Core.Enums.ResourcePurpose.Web));
    ...
    private static string GetDefaultConfigDirectory()
    {
        ...
        // Navigate to the TrialMatch config directory from the assembly location
        ...
        {
            Path.Combine(currentDir, "TrialMatch", "config"),
            Path.Combine(currentDir, "AppInfraCdkV1.Apps", "TrialMatch", "config"),
            Path.Combine(currentDir, "..", "..", "..", "AppInfraCdkV1.Apps", "TrialMatch", "config"),
            Path.Combine(Directory.GetCurrentDirectory(), "AppInfraCdkV1.Apps", "TrialMatch", "config")
        };
        ...
        return Path.Combine(currentDir, "TrialMatch", "config");
    }
